    ...
    public function process(Request $request)
{
    ...
    try {
        // Validar la fecha proporcionada y otros datos
        $validatedData = $request->validate([
            'delivery_date' => 'required|date_format:Y-m-d H:i:s', // Cambiar el formato de fecha si es necesario
        ...
        $order->created_at = Carbon::createFromFormat('Y-m-d H:i:s', $request->delivery_date); // Asignar la fecha proporcionada
        $order->save();
        ...
        // Cobrar el pago
        Charge::create([
            'amount' => $total * 100, // Monto en centavos (Stripe espera el monto en centavos)
            'currency' => 'usd',
